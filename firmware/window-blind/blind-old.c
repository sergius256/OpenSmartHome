  ...
  for(;;)
	...
	case 0: // Initialize system
	  ...
	  for(;;MovementTimeFull++)
	    {
	      _delay_ms(10);
	      if(!(PIND & 0x20)) //0b0010 0000)) // END2 switch _BV(PD5))) // 
		{
		  _delay_ms(50);
		  MovementTimeFull+=5;
		  if(!(PIND & 0x20)) // 0b00100000)) // _BV(PD5))) // 
		    break;
		}
	    }
...
void ParserSetup(void)
{
  ...
  ParserFunctions[3].handler=DoReset;
	...
	case 2:
	  // _delay_ms(10);
	  // MovementTimeCurrent++;
	  if(key_press & 0x20) // 0x10 // END1 switch _BV(PD4))) //
	    {
	      key_press=0;
	      FSM_State=3;
	    }
	...
	case 4:
	  // _delay_ms(10);
	  // MovementTimeFull++;
	  if(key_press & 0x10) // 0x20 0b0010 0000)) // END2 switch _BV(PD5))) // 
	    {
	      key_press=0;
	      FSM_State=1;
	    }
	...
	case 5:
	  ...
	  FSM_State=6; // Wait for event
	case 6:
	  if(key_press & 0x04) // UP switch
	    {
	      DesiredPosition=0;
	      key_press=0;
	    }
	  if(key_press & 0x80) // DOWN switch
	    {
	      DesiredPosition=100;
	      key_press=0;
	    }
	...
	case 8:
	  ...
	  if(CurrentPosition == DesiredPosition)
	    FSM_State=9;
	  if(key_press & 0x10) // END1 switch reached
	    {
	      MovementTimeCurrent=0; // We reached the beginning
	      key_press=0;
	      FSM_State=9;
	  ...
	  if(key_press & 0x04) // UP or DOWN switch pressed
	    {
	      key_press=0;
	      FSM_State=9;
	    }
	...
	case 11:
	  ...
	  if(CurrentPosition == DesiredPosition)
	    FSM_State=9;
	  if(key_press & 0x20) // END2 switch reached
	    {
	      MovementTimeCurrent=MovementTimeFull; // We reached the end
	      key_press=0;
	      FSM_State=9;
